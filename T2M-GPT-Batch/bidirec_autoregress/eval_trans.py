...
def evaluation_transformer(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, clip_model, eval_wrapper, draw = True, save = True, savegif=False, num_repeat=1) : 
    ...
    for batch in tqdm(val_loader):
        ...
        for i in range(num_repeat):
            ...
            m_token_len = torch.ceil(m_length/4).long()
            seq_token = math.ceil(seq/4)
            index_motion = trans(feat_clip_text, m_token_len, seq_token, True, type="sample")
            for k in range(bs):
                motion_idx = index_motion[k:k+1, :int(m_token_len[k].item())]
                pred_pose = net(motion_idx, type='decode')
                pred_pose_eval[k:k+1, :pred_pose.shape[1]] = pred_pose
            et_pred, em_pred = eval_wrapper.get_co_embeddings(word_embeddings, pos_one_hots, sent_len, pred_pose_eval, m_length)
