...
def get_attn_mask(_range=3, _max=8):
    mask = torch.zeros((1, 1, _max,_max), dtype=torch.bool)
    # Text mask
    # mask[0, 0, :, 0] = True
    # mask[0, 0, 0, :] = True
    i = 0
    while i+_range <= mask.shape[-1]:
        mask[0, 0, i:i+_range, i:i+_range] = True
        i += 1
    return mask
mask_temp = get_attn_mask(_range=3, _max=16)
...
class Text2Motion_Transformer(nn.Module):
    ...
    def sample(self, clip_feature, word_emb, m_length=None, if_test=False, rand_pos=False, CFG=-1, token_cond=None, max_steps = 10):
        max_length = 49
