...
def evaluation_transformer(out_dir, val_loader, net, trans, logger, writer, nb_iter, best_fid, best_iter, best_div, best_top1, best_top2, best_top3, best_matching, clip_model, eval_wrapper, draw = True, save = True, savegif=False) : 
    ...
    for i in range(1):
        for batch in tqdm(val_loader):
            ...
            # cb_idx_mask = generate_src_mask(index_motion.shape[1], lengths)
            # # [TODO] Make sue "0" token does't effect the decoder. Ps. T2M-GPT decode one sample at a time so 
            # index_motion[~cb_idx_mask] = 0
            # index_motion = index_motion[:, :-1] # [INFO] rm last token
            print('index_motion:', index_motion.max())
            import pdb; pdb.set_trace()
            lengths = torch.clip(lengths*4, max=196)
            for k in range(bs):
                pred_pose = net(index_motion[k:k+1], type='decode')
                cur_len = lengths[k].int().item() #pred_pose.shape[1]
