...
from noend_token import dataset_TM_train
import noend_token.eval_trans as eval_trans
import noend_token.t2m_trans as trans
##### ---- Exp dirs ---- #####
...
for nb_iter in tqdm(range(1, args.total_iter + 1), position=0, leave=True):
    ...
    # Time step masking
    mask_id = get_model(net).vqvae.num_code + 1
